...
def create_todo(todo: schemas.TodoCreate, user_id: int):
    date = todo.date
    title = todo.title
    ...
    mycursor.execute(
        insert_query, (date, title, contents, place, due_date, is_completed, user_id)
    )
...
def get_todo(db_connection: connection.MySQLConnection, todo_id: int):
    cursor = db_connection.cursor()
...
def delete_todo(db_connection: connection.MySQLConnection, todo_id: int):
    cursor = db_connection.cursor()
...
def get_todos(
    db_connection: connect.MySQLConnection, date: datetime.date, user_id: int
): ...
